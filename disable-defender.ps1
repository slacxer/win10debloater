...
#    $shortcut.Save()
    ...
    if($IsSystem) {
        ...
        Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows Defender\Real-Time Protection" -Name SpyNetReporting -Value 0 #default 2
        # Automatic Sample submission
        Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows Defender\Real-Time Protection" -Name SubmitSamplesConsent -Value 0 #default 2
        # Tamper protection
        Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows Defender\Features" -Name TamperProtection -Value 0 #default 5
        
        # Disable in registry
		$regwdf_list = @("DisableAntiSpyware", "DisableAntiVirus") #Default DisableAntiVirus 0, DisableAntiSpyware 0
		foreach($regwdf in $regwdf_list) {
			if($(Test-Path "HKLM:\SOFTWARE\Microsoft\Windows Defender")) {
                if( $(Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows Defender").$regwdf -ge 0) {
					if( $(Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows Defender").$regwdf -eq 1) {
                        Write-Host "        [i] Registry $regwdf already disabled"
                    } else {
                        Write-Host "        [i] Disable Registry $regwdf (next reboot)"
                        Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows Defender" -Name $regwdf -Value 1
                        
                    }
				} else {
				    Write-Host "        [i] Creating Registry $regwdf (need reboot to take effect)"	
					New-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows Defender" -Name $regwdf -Value 1 -PropertyType DWord -force
					
				}	
            } else {
				if($(Test-Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender")) {
					if( $(Get-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender").$regwdf -ge 0) {
                        if( $(Get-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender").$regwdf -eq 1) {
                            Write-Host "        [i] Registry Policies $regwdf already disabled"
                        } else {
                            Write-Host "        [i] Disable Registry Policies $regwdf (next reboot)"
                            Set-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender" -Name $regwdf -Value 1
                            
                        }
					} else {
						Write-Host "        [i] Creating Registry Policies $regwdf (need reboot to take effect)"	
					    New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender" -Name $regwdf -Value 1 -PropertyType DWord -force
					    
					}
                } else {			
                    Write-Host "        [i] Registry Key for $regwdf Does Not Exist"
				}
            }
		}
    ...
    if($MyInvocation.UnboundArguments -And $($MyInvocation.UnboundArguments.tolower().Contains("-delete"))) {
        ...
        # Rename files
        Rename-Item -Force -Path "C:\ProgramData\Windows\Windows Defender" -NewName WDF
        Rename-Item -Force -Path "C:\ProgramData\Windows\Windows Defender Advanced Threat Protection" -NewName WDFATP

        # Rename drivers
        Rename-Item -Force -Path  "C:\Windows\System32\drivers\wd" -NewName WDFD

        # Rename service registry entries
        foreach($svc in $svc_list) {
            Rename-Item -Force -Path "HKLM:\SYSTEM\CurrentControlSet\Services\$svc" -NewName "$svc.bak"
        }

        # Rename drivers registry entries
        foreach($drv in $drv_list) {
            Rename-Item -Force -Path "HKLM:\SYSTEM\CurrentControlSet\Services\$drv" -NewName "$drv.bak"
        }
    }
}
