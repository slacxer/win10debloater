...
if($(Test-Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\DataCollection")) {
        if( $(Get-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\DataCollection").AllowTelemetry -eq 0) {
            Write-Host "        [i] Service AllowTelemetry already disabled"
        } else {
            Write-Host "        [i] Disable service AllowTelemetry (next reboot)"
            Set-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\DataCollection" -Name Start -Value 0
            $need_reboot = $true
        }
} else {
        Write-Host "        [i] Telemetry key already deleted"
}
